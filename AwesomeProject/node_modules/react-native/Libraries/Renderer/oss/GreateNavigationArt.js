...
function getNativeShortTag(stateNode) {
    if (stateNode) {
        if (stateNode && stateNode.name === 'ReactNativeFiberHostComponent') {
...
function getReactNativeFiberHostComponentTag(nativeFiberHostComponent) {
    if (nativeFiberHostComponent) {
        ...
        if (_children) {
            let childrenTag = _children.map((child, index) => (child.name === 'ReactNativeFiberHostComponent' ? `${getReactNativeFiberHostComponentTag(child)}` : child)).join();
            if (childrenTag) {
...
function dumpFiberTree(roots, instanceCache, create, desc, currentFiberID = -1) {
    const fibers = [];
    ...
    const doms = instanceCache ? Object.keys(instanceCache).map((key)=> (instanceCache[key])).map((fiber: FiberNode) => {
        ...
        const children = stateNode&&stateNode.name === 'ReactNativeFiberHostComponent' ? stateNode._children.map((child) => (getNativeShortTag(child))).filter((child) => (child)) : null;
