...
function completeUnitOfWork(workInProgress) {
    ...
    hookCompleteUnitOfWorkByDumpFiberTree();
...
function performWorkOnRoot(root, expirationTime, isYieldy) {
    ...
    // debugger;
    hookPerformWorkOnRootIndexByDumpFiberTree();
  invariant(
    !isRendering,
...
function travelFiber(fiber: FiberNode, fibers: Array, fiberIDsSet: Set) {
    if (fiber && !fiberIDsSet.has(fiber.id)) {
        fiberIDsSet.add(fiber.id);
        fibers.push({
            id: fiber.id,
            tag: tag2String(fiber.tag),
            effectTag: effectTag2String(fiber.effectTag),
            type: fiber.type ? (typeof (fiber.type) === 'string' ? fiber.type : (fiber.type.name ? fiber.type.name : (fiber.type.displayName ? fiber.type.displayName : String(fiber.type.$$typeof)))) : '',
...
let performWorkOnRootIndex = -1;
let completeUnitOfWorkIndex = -1;

function hookPerformWorkOnRootIndexByDumpFiberTree() {
    performWorkOnRootIndex ++;
    completeUnitOfWorkIndex = -1;
}

function hookCompleteUnitOfWorkByDumpFiberTree() {
    completeUnitOfWorkIndex ++;
}

function dumpFiberTree(desc) {
    const fibers = [];
    const fiberIDsSet = new Set();
    for (let value of roots.values()) {
        const {current, finishedWork} = value;
        travelFiber(current, fibers, fiberIDsSet);
        travelFiber(finishedWork, fibers, fiberIDsSet);
    }
    // debugger;
    const doms = instanceCache ? Object.keys(instanceCache).map((key)=> (instanceCache[key])).map((fiber: FiberNode) => {
        const {stateNode} = fiber;
        const name = tag2String(fiber.tag);
    ...
    const fiberTreeInfo = {
        ...
        desc: `${desc}【performWorkOnRoot_${performWorkOnRootIndex}-completeUnitOfWork_${completeUnitOfWorkIndex}】`,
        fibers,
        doms
    };
    sendFiberTree(fiberTreeInfo);
    console.log('SSU', 'dumpFiberTree()', {roots, instanceCache, doms}, JSON.stringify(fiberTreeInfo));
    // debugger;
}
