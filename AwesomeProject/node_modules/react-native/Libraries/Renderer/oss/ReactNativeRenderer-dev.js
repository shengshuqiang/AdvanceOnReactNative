 ...
 */
const renderSources = [];
function pushRenderSource(methodName) {
  renderSources.push(methodName);
}
function popAllRenderSource() {
  const result = renderSources.join('-');
  renderSources.splice(0, renderSources.length);
  return result;
}
...
function effectTag2String(effectTag: number) {
    ...
    if (primaryEffectTag === Placement) {
        effectTagStr += 'Placement, ';
    } else if (primaryEffectTag === Update) {
        effectTagStr += 'Update, ';
    } else if (primaryEffectTag === PlacementAndUpdate) {
        effectTagStr += 'PlacementAndUpdate, ';
    } else if (primaryEffectTag === Deletion) {
        effectTagStr += 'Deletion, ';
    }
    if ((effectTag & ContentReset) !== NoEffect) {
        effectTagStr += 'ContentReset, ';
    }
    if ((effectTag & Callback) !== NoEffect) {
        effectTagStr += 'Callback, ';
    }
    if ((effectTag & DidCapture) !== NoEffect) {
        effectTagStr += 'DidCapture, ';
    }
    if ((effectTag & Ref) !== NoEffect) {
        effectTagStr += 'Ref, ';
    }
    if ((effectTag & Snapshot) !== NoEffect) {
        effectTagStr += 'Snapshot, ';
    }
    if ((effectTag & Passive) !== NoEffect) {
        effectTagStr += 'Passive, ';
    }
    if (effectTagStr.length) {
        // 去掉尾部", "
        effectTagStr = effectTagStr.substring(0, effectTagStr.length - 2);
    }
    return effectTagStr;
}
...
function scheduleDeferredCallback$1(callback, options) {
    pushRunRecordStack('scheduleDeferredCallback$1');
    try {
...
function createInstance(
  ...
  internalInstanceHandle
    ...
    pushRunRecordStack('createInstance');
    try {
        var tag = allocateTag();
...
function createTextInstance(
  ...
  internalInstanceHandle
    ...
    pushRunRecordStack('createTextInstance');
    try {
        invariant(
...
function finalizeInitialChildren(
  ...
  hostContext
    ...
    pushRunRecordStack('finalizeInitialChildren');
    try {
...
function appendChild(parentInstance, child) {
    pushRunRecordStack('appendChild');
    try {
...
function appendChildToContainer(parentInstance, child) {
    pushRunRecordStack('appendChildToContainer');
    try {
        var childTag = typeof child === "number" ? child : child._nativeTag;
...
function commitTextUpdate(textInstance, oldText, newText) {
    pushRunRecordStack('commitTextUpdate');
    try {
        UIManager.updateView(
...
function commitUpdate(
  ...
  internalInstanceHandle
    ...
    pushRunRecordStack('commitUpdate');
    try {
...
function insertBefore(parentInstance, child, beforeChild) {
    pushRunRecordStack('insertBefore');
    try {
        var children = parentInstance._children;
        var index = children.indexOf(child);
...
function insertInContainerBefore(parentInstance, child, beforeChild) {
    pushRunRecordStack('insertInContainerBefore');
    try {
...
function popTopLevelContextObject(fiber) {
    pushRunRecordStack('popTopLevelContextObject');
    try {
        pop(didPerformWorkStackCursor, fiber);
        pop(contextStackCursor, fiber);
    } finally {
...
function onCommitRoot(root) {
    pushRunRecordStack('onCommitRoot');
    try {
...
function onCommitUnmount(fiber) {
    pushRunRecordStack('onCommitUnmount');
    try {
...
function FiberNode(tag, pendingProps, key, mode) {
    ...
    try {
        ...
        // Effects
        pushPopRunRecordStack(`this.effectTag = NoEffect`);
        ...
        pushPopRunRecordStack(`expirationTime=(${this.id}, ${this.expirationTime})`);
        this.childExpirationTime = NoWork;
        pushPopRunRecordStack(`childExpirationTime=(${this.id}, ${this.childExpirationTime})`);
...
FiberNode.prototype.toString = function() {
    return this.id + "_" + tag2String(this.tag) + "_" + effectTag2String(this.effectTag);
}
...
var createFiber = function(tag, pendingProps, key, mode) {
  // $FlowFixMe: the shapes are exact here but Flow doesn't like constructors
    pushRunRecordStack(`createFiber(${tag2String(tag)}, ${fiberID})`);
    try {
...
function shouldConstruct(Component) {
    pushRunRecordStack('shouldConstruct');
    try {
...
function createWorkInProgress(current, pendingProps, expirationTime) {
    pushRunRecordStack(`createWorkInProgress(${current.id},${current.alternate ? current.alternate.id : -1})`);
    try {
        var workInProgress = current.alternate;
        if (workInProgress === null) {
            pushRunRecordStack(`workInProgress===null`);
            try {
            ...
            try {
                ...
                pushPopRunRecordStack(`workInProgress.effectTag = NoEffect`);
        ...
        pushPopRunRecordStack(`childExpirationTime=(${workInProgress.id}, ${workInProgress.childExpirationTime})`);
        workInProgress.expirationTime = current.expirationTime;
        pushPopRunRecordStack(`expirationTime=(${workInProgress.id}, ${workInProgress.expirationTime})`);
...
function createFiberFromTypeAndProps(
  ...
  expirationTime
    ...
    pushRunRecordStack('createFiberFromTypeAndProps');
    try {
        var fiber = void 0;
        ...
        pushPopRunRecordStack(`expirationTime=(${fiber.id}, ${fiber.expirationTime})`);
...
function createFiberFromElement(element, mode, expirationTime) {
    pushRunRecordStack(`createFiberFromElement(${element2String(element)})`);
    try {
        var owner = null;
        {
...
function createFiberFromFragment(elements, mode, expirationTime, key) {
    pushRunRecordStack('createFiberFromFragment');
    try {
...
function createFiberFromProfiler(pendingProps, mode, expirationTime, key) {
    pushRunRecordStack('createFiberFromProfiler');
    try {
        {
            if (
...
function createFiberFromMode(pendingProps, mode, expirationTime, key) {
    pushRunRecordStack('createFiberFromMode');
    try {
...
function createFiberFromSuspense(pendingProps, mode, expirationTime, key) {
    pushRunRecordStack('createFiberFromSuspense');
    try {
...
function createFiberFromText(content, mode, expirationTime) {
    pushRunRecordStack('createFiberFromText');
    try {
...
function createFiberFromHostInstanceForDeletion() {
    pushRunRecordStack(`createFiberFromHostInstanceForDeletion`);
    try {
...
function createFiberFromPortal(portal, mode, expirationTime) {
    pushRunRecordStack(`createFiberFromPortal`);
    try {
...
function assignFiberPropertiesInDEV(target, source) {
    pushRunRecordStack(`assignFiberPropertiesInDEV`);
    try {
        if (target === null) {
        ...
        pushPopRunRecordStack(`expirationTime=(${target.id}, ${target.expirationTime})`);
        target.childExpirationTime = source.childExpirationTime;
        pushPopRunRecordStack(`childExpirationTime=(${target.id}, ${target.childExpirationTime})`);
...
function createFiberRoot(containerInfo, isConcurrent, hydrate) {
    pushRunRecordStack('createFiberRoot');
    try {
...
function findNextExpirationTimeToWorkOn(completedExpirationTime, root) {
    pushRunRecordStack(`findNextExpirationTimeToWorkOn`)
    try {
        var earliestSuspendedTime = root.earliestSuspendedTime;
        var latestSuspendedTime = root.latestSuspendedTime;
        var earliestPendingTime = root.earliestPendingTime;
        ...
        pushPopRunRecordStack(`expirationTime=(root, ${root.expirationTime})`);
    } finally {
        popRunRecordStack();
    }
}
...
function applyDerivedStateFromProps(
  ...
  nextProps
    ...
    pushRunRecordStack('applyDerivedStateFromProps');
    try {
...
var classComponentUpdater = {
  ...
  enqueueSetState: function(inst, payload, callback) {
      pushRunRecordStack(`enqueueSetState`);
      try {
  ...
  enqueueReplaceState: function(inst, payload, callback) {
      pushRunRecordStack(`enqueueReplaceState`);
      try {
          var fiber = get$1(inst);
          var currentTime = requestCurrentTime();
          var expirationTime = computeExpirationForFiber(currentTime, fiber);
  ...
  enqueueForceUpdate: function(inst, callback) {
      pushRunRecordStack(`enqueueForceUpdate`);
      try {
          var fiber = get$1(inst);
          var currentTime = requestCurrentTime();
          var expirationTime = computeExpirationForFiber(currentTime, fiber);
...
function checkShouldComponentUpdate(
  ...
  nextContext
    ...
    pushRunRecordStack('checkShouldComponentUpdate');
    try {
        var instance = workInProgress.stateNode;
...
function hookClassInstance(instance) {
    if (instance) {
        const {
            constructor,
            getDerivedStateFromProps,
        ...
        if (getDerivedStateFromProps) {
            instance.getDerivedStateFromProps = function (nextProps, prevState) {
        ...
        if (UNSAFE_componentWillMount) {
            instance.UNSAFE_componentWillMount = function () {
        ...
        if (componentWillReceiveProps) {
            instance.componentWillReceiveProps = function (newProps, nextContext) {
        ...
        if (UNSAFE_componentWillReceiveProps) {
            instance.UNSAFE_componentWillReceiveProps = function (newProps, nextContext) {
        ...
        if (shouldComponentUpdate) {
            instance.shouldComponentUpdate = function (newProps, newState, nextContext) {
        ...
        if (componentWillUpdate) {
            instance.componentWillUpdate = function (newProps, newState, nextContext) {
        ...
        if (UNSAFE_componentWillUpdate) {
            instance.UNSAFE_componentWillUpdate = function (newProps, newState, nextContext) {
        ...
        if (getSnapshotBeforeUpdate) {
            instance.getSnapshotBeforeUpdate = function (prevProps, prevState) {
        ...
        if (isReactComponent) {
            instance.isReactComponent = function () {
...
function constructClassInstance(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('constructClassInstance');
    try {
...
function callComponentWillMount(workInProgress, instance) {
    pushRunRecordStack('callComponentWillMount');
    try {
...
function callComponentWillReceiveProps(
  ...
  nextContext
    ...
    pushRunRecordStack('callComponentWillReceiveProps');
    try {
        var oldState = instance.state;
...
function mountClassInstance(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('mountClassInstance');
    try {
        {
        ...
        if (typeof instance.componentDidMount === "function") {
            pushPopRunRecordStack(`workInProgress.effectTag |= Update`);
...
function resumeMountClassInstance(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('resumeMountClassInstance');
    try {
        var instance = workInProgress.stateNode;
...
function updateClassInstance(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateClassInstance');
    try {
        var instance = workInProgress.stateNode;
        ...
        if (
            ...
            if (typeof instance.getSnapshotBeforeUpdate === "function") {
                if (
                    ...
                    pushPopRunRecordStack(`workInProgress.effectTag |= Snapshot`);
...
function ChildReconciler(shouldTrackSideEffects) {
    // pushRunRecordStack('ChildReconciler');
    // try {
        function deleteChild(returnFiber, childToDelete) {
            pushRunRecordStack('deleteChild');
            try {
                ...
                pushPopRunRecordStack(`childToDelete.effectTag = Deletion`);
        ...
        function deleteRemainingChildren(returnFiber, currentFirstChild) {
            pushRunRecordStack('deleteRemainingChildren');
            try {
                if (!shouldTrackSideEffects) {
                    // Noop.
                    return null;
            ...
            try {
                ...
                if (current$$1 !== null) {
                    ...
                    if (oldIndex < lastPlacedIndex) {
                        // This is a move.
                        pushPopRunRecordStack(`newFiber.effectTag = Placement`);
        ...
        function updateTextNode(
            ...
            pushRunRecordStack('updateTextNode');
            try {
        ...
        function updateElement(returnFiber, current$$1, element, expirationTime) {
            pushRunRecordStack('updateElement');
            try {
        ...
        function createChild(returnFiber, newChild, expirationTime) {
            pushRunRecordStack('createChild');
            try {
        ...
        function reconcileChildrenArray(
            ...
            pushRunRecordStack('reconcileChildrenArray');
            try {
        ...
        function reconcileChildrenIterator(
            ...
            pushRunRecordStack('reconcileChildrenIterator');
            try {
        ...
        function reconcileSingleTextNode(
            ...
            pushRunRecordStack('reconcileSingleTextNode');
            try {
        ...
        function reconcileSingleElement(
            ...
            pushRunRecordStack('reconcileSingleElement');
            try {
                var key = element.key;
                ...
                while (child !== null) {
                    ...
                    try {
                        pushRunRecordStack(`child({${child.key}, ${tag2String(child.tag)}, ${getFiberType(child)}}, {${element.key}, ${getElementType(element)}})`);
                        try {
                            if (child.key === key) {
                                pushRunRecordStack('child.key === key');
                                try {
                                    if (
                                        child.tag === Fragment
                                        ...
                                        pushRunRecordStack('...child.elementType === element.type');
                                        try {
                        ...
                        dumpFiberTree('reconcileSingleElement', child.id);
                    }
                    child = child.sibling;
                }
        ...
        function reconcileChildFibers(
            ...
            pushRunRecordStack(`reconcileChildFibers(${String(typeof newChild)})`);
            try {
                ...
                if (isObject) {
                    pushRunRecordStack('typeof newChild === "object"');
                    try {
                        pushRunRecordStack(`switch(${String(newChild.$$typeof)})`);
                        try {
                            switch (newChild.$$typeof) {
                                case REACT_ELEMENT_TYPE:
                                    pushRunRecordStack('placeSingleChild');
                                    try {
                ...
                if (typeof newChild === "string" || typeof newChild === "number") {
                    pushRunRecordStack('typeof newChild === "string、number"');
                    try {
                        pushRunRecordStack('placeSingleChild');
                        try {
                            return placeSingleChild(
                ...
                if (isArray(newChild)) {
                    pushRunRecordStack('isArray(newChild)');
                    try {
                ...
                if (getIteratorFn(newChild)) {
                    pushRunRecordStack('getIteratorFn(newChild)');
                    try {
                ...
                if (typeof newChild === "undefined" && !isUnkeyedTopLevelFragment) {
                    ...
                    pushRunRecordStack('typeof newChild === "undefined" && !isUnkeyedTopLevelFragment');
                    try {
...
function cloneChildFibers(current$$1, workInProgress) {
  pushRunRecordStack('cloneChildFibers');
  try {
    invariant(
...
function popHostContainer(fiber) {
    pushRunRecordStack('popHostContainer');
    try {
        pop(contextStackCursor$1, fiber);
...
function popHostContext(fiber) {
    pushRunRecordStack('popHostContext');
    try {
...
function renderWithHooks(
  ...
  nextRenderExpirationTime
    ...
    pushRunRecordStack('renderWithHooks');
    try {
        ...
        pushRunRecordStack('children = Component()');
        try {
        ...
        pushPopRunRecordStack(`expirationTime=(${renderedWork.id}, ${renderedWork.expirationTime})`);
        renderedWork.updateQueue = componentUpdateQueue;
        pushPopRunRecordStack(`renderedWork.effectTag |= sideEffectTag`);
...
function bailoutHooks(current, workInProgress, expirationTime) {
    pushRunRecordStack('bailoutHooks');
    try {
        workInProgress.updateQueue = current.updateQueue;
        pushPopRunRecordStack(`workInProgress.effectTag &= ~(Passive | Update)`);
        workInProgress.effectTag &= ~(Passive | Update);
        if (current.expirationTime <= expirationTime) {
            current.expirationTime = NoWork;
            pushPopRunRecordStack(`expirationTime=(${current.id}, ${current.expirationTime})`);
        }
    } finally {
        popRunRecordStack();
    }
...
function insertNonHydratedInstance(returnFiber, fiber) {
    pushPopRunRecordStack(`fiber.effectTag |= Placement`);
...
function prepareToHydrateHostInstance(
  ...
  hostContext
    ...
    pushRunRecordStack('prepareToHydrateHostInstance');
    try {
        if (!supportsHydration) {
            invariant(
                false,
...
function popHydrationState(fiber) {
    pushRunRecordStack('popHydrationState');
    try {
        if (!supportsHydration) {
            return false;
        }
...
function reconcileChildren(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('reconcileChildren');
    try {
        if (current$$1 === null) {
...
function forceUnmountCurrentAndReconcile(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('forceUnmountCurrentAndReconcile');
    try {
...
function updateForwardRef(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateForwardRef');
    try {
        ...
        prepareToReadContext(workInProgress, renderExpirationTime);
        {
            ...
            pushRunRecordStack('nextChildren = renderWithHooks()');
            try {
        ...
        pushPopRunRecordStack(`workInProgress.effectTag |= PerformedWork`);
...
function updateMemoComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateMemoComponent');
    try {
        if (current$$1 === null) {
...
function updateSimpleMemoComponent(
  ...
  // We'll need to figure out if this is fine or can cause issues.
    pushRunRecordStack('updateSimpleMemoComponent');
    try {
        {
            if (workInProgress.type !== workInProgress.elementType) {
                // Lazy component props can't be validated in createElement
...
function updateFragment(current$$1, workInProgress, renderExpirationTime) {
    pushRunRecordStack('updateFragment');
    try {
...
function updateProfiler(current$$1, workInProgress, renderExpirationTime) {
    pushRunRecordStack('updateProfiler');
    try {
        if (enableProfilerTimer) {
            pushPopRunRecordStack(`workInProgress.effectTag |= Update`);
            workInProgress.effectTag |= Update;
...
function markRef(current$$1, workInProgress) {
  ...
  if (
    ...
    // Schedule a Ref effect
      pushPopRunRecordStack(`workInProgress.effectTag |= Ref`);
...
function updateFunctionComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateFunctionComponent');
    try {
        {
            if (workInProgress.type !== workInProgress.elementType) {
                // Lazy component props can't be validated in createElement
...
function updateClassComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateClassComponent');
    try {
        {
            if (workInProgress.type !== workInProgress.elementType) {
                // Lazy component props can't be validated in createElement
        ...
        if (instance === null) {
            pushRunRecordStack('instance === null');
            try {
                if (current$$1 !== null) {
                    ...
                    pushPopRunRecordStack(`workInProgress.effectTag |= Placement`);
            ...
            pushRunRecordStack('...current$$1 === null');
            try {
...
function finishClassComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('finishClassComponent');
    try {
        ...
        if (!shouldUpdate && !didCaptureError) {
            pushRunRecordStack('!shouldUpdate && !didCaptureError');
            try {
        ...
        if (
            ...
            {
                setCurrentPhase("render");
                pushRunRecordStack('nextChildren = instance.render()');
                try {
        ...
        if (current$$1 !== null && didCaptureError) {
            ...
            pushRunRecordStack('current$$1 !== null && didCaptureError');
            try {
...
function updateHostRoot(current$$1, workInProgress, renderExpirationTime) {
    pushRunRecordStack('updateHostRoot');
    try {
        ...
        pushRunRecordStack('nextChildren = nextState.element');
        try {
...
function updateHostComponent(current$$1, workInProgress, renderExpirationTime) {
    pushRunRecordStack('updateHostComponent');
    try {
        ...
        pushRunRecordStack('nextChildren = nextProps.children');
        try {
            var nextChildren = nextProps.children;
        } finally {
            popRunRecordStack();
        ...
        if (isDirectTextChild) {
            ...
            pushPopRunRecordStack(`workInProgress.effectTag |= ContentReset`);
...
function updateHostText(current$$1, workInProgress) {
    pushRunRecordStack('updateHostText');
    try {
        if (current$$1 === null) {
            tryToClaimNextHydratableInstance(workInProgress);
        }
...
function mountLazyComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('mountLazyComponent');
    try {
        if (_current !== null) {
        ...
        switch (resolvedTag) {
            case FunctionComponent: {
                ...
                pushRunRecordStack('child = updateFunctionComponent');
                try {
            ...
            case ClassComponent: {
                pushRunRecordStack('child = updateClassComponent');
                try {
            ...
            case ForwardRef: {
                pushRunRecordStack('child = updateForwardRef');
                try {
            ...
            case MemoComponent: {
                pushRunRecordStack('child = updateMemoComponent');
                try {
                    {
                        if (workInProgress.type !== workInProgress.elementType) {
...
function mountIncompleteClassComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('mountIncompleteClassComponent');
    try {
        if (_current !== null) {
...
function mountIndeterminateComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('mountIndeterminateComponent');
    try {
        if (_current !== null) {
...
function updateSuspenseComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateSuspenseComponent');
    try {
        ...
        if ((workInProgress.effectTag & DidCapture) === NoEffect) {
            ...
            nextDidTimeout = true;
            pushPopRunRecordStack(`workInProgress.effectTag &= ~DidCapture`);
        ...
        if (current$$1 === null) {
            ...
            if (prevDidTimeout) {
                ...
                if (nextDidTimeout) {
                    ...
                    pushPopRunRecordStack(`childExpirationTime=(${_primaryChildFragment.id}, ${_primaryChildFragment.childExpirationTime})`);
                    // Skip the primary children, and continue working on the
                    // fallback children.
                ...
                if (nextDidTimeout) {
                    ...
                    pushPopRunRecordStack(`_fallbackChildFragment2.effectTag |= Placement`);
                    _fallbackChildFragment2.effectTag |= Placement;
                    child = _primaryChildFragment2;
                    _primaryChildFragment2.childExpirationTime = NoWork;
                    pushPopRunRecordStack(`childExpirationTime=(${_primaryChildFragment2.id}, ${_primaryChildFragment2.childExpirationTime})`);
                    // Skip the primary children, and continue working on the
                    // fallback children.
...
function updateDehydratedSuspenseComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateDehydratedSuspenseComponent');
    try {
        if (current$$1 === null) {
        ...
        if (
            ...
            pushPopRunRecordStack(`current$$1.effectTag = Deletion`);
...
function updatePortalComponent(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updatePortalComponent');
    try {
        pushHostContainer(workInProgress, workInProgress.stateNode.containerInfo);
        pushRunRecordStack('nextChildren = workInProgress.pendingProps');
        try {
            var nextChildren = workInProgress.pendingProps;
        } finally {
            popRunRecordStack();
...
function updateContextProvider(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateContextProvider');
    try {
        ...
        pushRunRecordStack('newChildren = newProps.children');
        try {
...
function updateContextConsumer(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('updateContextConsumer');
    try {
...
function bailoutOnAlreadyFinishedWork(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('bailoutOnAlreadyFinishedWork');
    try {
        ...
        pushRunRecordStack(`expirationTime(${childExpirationTime},${renderExpirationTime})`);
        try {
            if (childExpirationTime < renderExpirationTime) {
                pushPopRunRecordStack('childExpirationTime<renderExpirationTime');
                ...
                // fibers and continue.
                pushRunRecordStack('childExpirationTime>=renderExpirationTime');
                try {
...
function beginWork(current$$1, workInProgress, renderExpirationTime) {
    pushRunRecordStack(`beginWork(${current$$1 ? current$$1.id : -1}, ${workInProgress ? workInProgress.id : -1}, ${workInProgress.expirationTime})`);
    try {
        ...
        if (current$$1 !== null) {
            ...
            try {
                ...
                if (oldProps !== newProps || hasContextChanged()) {
                    ...
                    pushRunRecordStack('oldProps !== newProps || hasContextChanged()');
                    try {
                        didReceiveUpdate = true;
                    } finally {
                        popRunRecordStack();
                    ...
                    pushRunRecordStack('...updateExpirationTime < renderExpirationTime');
                    try {
                        didReceiveUpdate = false;
            ...
            didReceiveUpdate = false;
            pushPopRunRecordStack('current$$1 === null');
        }
        ...
        // debugger;
        pushRunRecordStack(`switch (${tag2String(workInProgress.tag)})`);
        try {
            switch (workInProgress.tag) {
  ...
  while (node !== null) {
    ...
    if (node.childExpirationTime < renderExpirationTime) {
      node.childExpirationTime = renderExpirationTime;
        pushPopRunRecordStack(`childExpirationTime=(${node.id}, ${node.childExpirationTime})`);
        if (
        alternate !== null &&
        alternate.childExpirationTime < renderExpirationTime
      ) {
        alternate.childExpirationTime = renderExpirationTime;
            pushPopRunRecordStack(`childExpirationTime=(${alternate.id}, ${alternate.childExpirationTime})`);
        }
    } else if (
      alternate !== null &&
      alternate.childExpirationTime < renderExpirationTime
...
function propagateContextChange(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack(`propagateContextChange`);
    try {
        ...
        while (fiber !== null) {
            ...
            if (list !== null) {
                ...
                while (dependency !== null) {
                    ...
                    if (
                        ...
                        if (
                            ...
                            alternate.expirationTime = renderExpirationTime;
                            pushPopRunRecordStack(`expirationTime=(${alternate.id}, ${alternate.expirationTime})`);
                        }
                        ...
                        if (list.expirationTime < renderExpirationTime) {
                            ...
                            pushPopRunRecordStack(`expirationTime=(${list.id}, ${list.expirationTime})`);
                        }
                ...
                if (
                    ...
                    _alternate.expirationTime = renderExpirationTime;
                    pushPopRunRecordStack(`expirationTime=(${_alternate.id}, ${_alternate.expirationTime})`);
                }
...
function createUpdate(expirationTime) {
    pushRunRecordStack(`createUpdate`);
    try {
        return {
...
function enqueueUpdate(fiber, update) {
    pushRunRecordStack('enqueueUpdate');
    try {
...
function getStateFromUpdate(
  ...
  pushRunRecordStack('getStateFromUpdate');
  try {
    switch (update.tag) {
      case ReplaceState: {
      ...
      case CaptureUpdate: {
          pushPopRunRecordStack(`workInProgress.effectTag = (workInProgress.effectTag & ~ShouldCapture) | DidCapture`);
...
function processUpdateQueue(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('processUpdateQueue');
    try {
        ...
        while (update !== null) {
            ...
            if (updateExpirationTime < renderExpirationTime) {
                ...
                if (_callback !== null) {
                    pushPopRunRecordStack(`workInProgress.effectTag |= Callback`);
...
function checkHasForceUpdateAfterProcessing() {
    pushRunRecordStack('checkHasForceUpdateAfterProcessing');
    try {
...
function commitUpdateQueue(
  ...
  renderExpirationTime
    ...
    pushRunRecordStack('commitUpdateQueue');
    try {
...
function commitUpdateEffects(effect, instance) {
    pushRunRecordStack('commitUpdateEffects');
    try {
        while (effect !== null) {
...
if (supportsMutation) {
  ...
  appendAllChildren = function(
    ...
    isHidden
      ...
      pushRunRecordStack('appendAllChildren');
      try {
  ...
  updateHostContainer = function(workInProgress) {
      pushRunRecordStack('updateHostContainer');
      try {
          // Noop
      } finally {
          popRunRecordStack();
  ...
  updateHostComponent$1 = function(
    ...
    rootContainerInstance
      ...
      pushRunRecordStack('updateHostComponent$1');
      try {
  ...
  updateHostText$1 = function(current, workInProgress, oldText, newText) {
    // If the text differs, mark it as an update. All the work in done in commitWork.
      pushRunRecordStack('updateHostText$1');
      try {
          if (oldText !== newText) {
              markUpdate(workInProgress);
          }
...
function completeWork(current, workInProgress, renderExpirationTime) {
    pushRunRecordStack(`completeWork(current${current ? current.id : -1}, workInProgress${workInProgress ? workInProgress.id : -1})`);
    try {
        var newProps = workInProgress.pendingProps;
                ...
                case SuspenseComponent: {
                    ...
                    if (current === null) {
                        ...
                        if (currentFallbackChild !== null) {
                            ...
                            pushPopRunRecordStack(`currentFallbackChild.effectTag = Deletion`);
...
var callComponentWillUnmountWithTimer = function(current$$1, instance) {
    pushRunRecordStack('callComponentWillUnmountWithTimer');
    try {
...
function safelyCallComponentWillUnmount(current$$1, instance) {
    pushRunRecordStack('safelyCallComponentWillUnmount');
    try {
        {
...
function safelyCallDestroy(current$$1, destroy) {
    pushRunRecordStack('safelyCallDestroy');
    try {
        {
...
function commitBeforeMutationLifeCycles(current$$1, finishedWork) {
    pushRunRecordStack('commitBeforeMutationLifeCycles');
    try {
...
function commitHookEffectList(unmountTag, mountTag, finishedWork) {
    pushRunRecordStack('commitHookEffectList');
    try {
...
function commitLifeCycles(
  ...
  committedExpirationTime
    ...
    pushRunRecordStack('commitLifeCycles');
    try {
        pushRunRecordStack(`switch (${tag2String(finishedWork.tag)})`);
        try {
            switch (finishedWork.tag) {
                case FunctionComponent:
                case ForwardRef:
                ...
                case ClassComponent: {
                    var instance = finishedWork.stateNode;
                    pushRunRecordStack(`finishedWork.effectTag & Update`);
                    try {
                        ...
                        var updateQueue = finishedWork.updateQueue;
                        pushRunRecordStack(`updateQueue !== null`);
                        try {
                            if (updateQueue !== null) {
                                {
                                    if (
...
function commitUnmount(current$$1) {
    pushRunRecordStack('commitUnmount');
    try {
...
function commitNestedUnmounts(root) {
    pushRunRecordStack('commitNestedUnmounts');
    try {
...
function commitContainer(finishedWork) {
    pushRunRecordStack('commitContainer');
    try {
        if (!supportsPersistence) {
            return;
        }
...
function commitPlacement(finishedWork) {
    pushRunRecordStack('commitPlacement');
    try {
        if (!supportsMutation) {
            return;
        }
        ...
        if (parentFiber.effectTag & ContentReset) {
            ...
            pushPopRunRecordStack(`parentFiber.effectTag &= ~ContentReset`);
...
function unmountHostComponents(current$$1) {
    pushRunRecordStack('unmountHostComponents');
    try {
...
function commitDeletion(current$$1) {
    pushRunRecordStack('commitDeletion');
    try {
        if (supportsMutation) {
...
function throwException(
  ...
  // The source fiber did not complete.
    pushPopRunRecordStack(`sourceFiber.effectTag |= Incomplete`);
    ...
    do {
      if (
        ...
        if ((_workInProgress.mode & ConcurrentMode) === NoEffect) {
            pushPopRunRecordStack(`_workInProgress.effectTag |= DidCapture`);
            _workInProgress.effectTag |= DidCapture;

          // We're going to commit this fiber even though it didn't complete.
          // But we shouldn't call any lifecycle methods or callbacks. Remove
          // all lifecycle effect tags.
            pushPopRunRecordStack(`sourceFiber.effectTag &= ~(LifecycleEffectMask | Incomplete)`);
          ...
          // indicate that it still has pending work.
            ...
            pushPopRunRecordStack(`expirationTime=(${sourceFiber.id}, ${sourceFiber.expirationTime})`);
        ...
        renderDidSuspend(root, absoluteTimeoutMs, renderExpirationTime);
          pushPopRunRecordStack(`_workInProgress.effectTag |= ShouldCapture`);

        _workInProgress.effectTag |= ShouldCapture;
          _workInProgress.expirationTime = renderExpirationTime;
          pushPopRunRecordStack(`expirationTime=(${_workInProgress.id}, ${_workInProgress.expirationTime})`);
          return;
      } else if (
        enableSuspenseServerRenderer &&
    ...
    switch (workInProgress.tag) {
      case HostRoot: {
        var _errorInfo = value;
          pushPopRunRecordStack(`workInProgress.effectTag |= ShouldCapture`);
            ...
            switch (workInProgress.tag) {
                case ClassComponent: {
                    ...
                    if (effectTag & ShouldCapture) {
                        pushPopRunRecordStack(`workInProgress.effectTag = (effectTag & ~ShouldCapture) | DidCapture`);
                ...
                case HostRoot: {
                    ...
                    pushPopRunRecordStack(`workInProgress.effectTag = (_effectTag & ~ShouldCapture) | DidCapture`);
                ...
                case SuspenseComponent: {
                    ...
                    if (_effectTag2 & ShouldCapture) {
                        pushPopRunRecordStack(`workInProgress.effectTag = (_effectTag2 & ~ShouldCapture) | DidCapture`);
                ...
                case DehydratedSuspenseComponent: {
                    if (enableSuspenseServerRenderer) {
                        ...
                        if (_effectTag3 & ShouldCapture) {
                            pushPopRunRecordStack(`workInProgress.effectTag = (_effectTag3 & ~ShouldCapture) | DidCapture`);
                            workInProgress.effectTag =
...
function commitAllHostEffects() {
    pushRunRecordStack('commitAllHostEffects');
    try {
        console.log('SSU', 'ReactNativeRenderer-dev.js#commitAllHostEffects()');

        // debugger;
        while (nextEffect !== null) {
            pushRunRecordStack(`nextEffect(${nextEffect.id}, ${effectTag2String(nextEffect.effectTag)})`);
            try {
                {
                ...
                pushRunRecordStack(`effectTag & ContentReset`);
                try {
                ...
                pushRunRecordStack(`effectTag & Ref`);
                try {
                    if (effectTag & Ref) {
                ...
                var primaryEffectTag = effectTag & (Placement | Update | Deletion);
                pushRunRecordStack(`primaryEffectTag(${effectTag2String(primaryEffectTag)})`);
                try {
                    switch (primaryEffectTag) {
                        case Placement: {
                            ...
                            // to kill this.
                            pushPopRunRecordStack(`nextEffect.effectTag &= ~Placement`);
    ...
    try {
        while (nextEffect !== null) {
            ...
            try {
                ...
                var effectTag = nextEffect.effectTag;
                pushRunRecordStack(`effectTag & Snapshot`);
                try {
...
function commitAllLifeCycles(finishedRoot, committedExpirationTime) {
    pushRunRecordStack('commitAllLifeCycles');
    try {
        {
        ...
        while (nextEffect !== null) {
            ...
            try {
                ...
                pushRunRecordStack(`effectTag & (Update | Callback)`);
                try {
                ...
                pushRunRecordStack(`effectTag & Passive`);
                try {
...
function flushPassiveEffects() {
    pushRunRecordStack('flushPassiveEffects');
    try {
...
function commitRoot(root, finishedWork) {
    pushRunRecordStack('commitRoot');
    try {
        // dumpFiberTree('commitRoot');
        ...
        try {
            ...
            pushRunRecordStack('root.current = finishedWork');
            try {
                root.current = finishedWork;
            } finally {
                popRunRecordStack();
            ...
            try {
                ...
                if ((workInProgress.effectTag & Incomplete) === NoEffect) {
                    ...
                    if (next !== null) {
                        ...
                        // from the effect tag.
                        pushPopRunRecordStack(`next.effectTag &= HostEffectMask`);
                    ...
                    if (returnFiber !== null) {
                        ...
                        pushPopRunRecordStack(`returnFiber.effectTag |= Incomplete`);
...
function performUnitOfWork(workInProgress) {
    pushRunRecordStack('performUnitOfWork');
    try {
        // The current, flushed, state of this fiber is the alternate.
        // Ideally nothing should rely on this, but relying on it here
        // means that we don't need an additional field on the work in
...
function renderRoot(root, isYieldy) {
    pushRunRecordStack(`renderRoot${root.nextExpirationTimeToWorkOn}`);
    try {
        dumpFiberTree('renderRoot', root.current.id);
        invariant(
            !isWorking,
...
function computeExpirationForFiber(currentTime, fiber) {
    pushRunRecordStack('computeExpirationForFiber');
    try {
...
function scheduleWorkToRoot(fiber, expirationTime) {
    pushRunRecordStack(`scheduleWorkToRoot`);
    try {
...
function scheduleWork(fiber, expirationTime) {
    pushRunRecordStack('scheduleWork');
    try {
...
function scheduleCallbackWithExpirationTime(root, expirationTime) {
    pushRunRecordStack('scheduleCallbackWithExpirationTime');
    try {
...
function onFatal(root) {
    pushRunRecordStack('onFatal');
    try {
        pushRunRecordStack('root.finishedWork = null');
        try {
...
function onComplete(root, finishedWork, expirationTime) {
    pushRunRecordStack('onComplete');
    try {
        root.pendingCommitExpirationTime = expirationTime;
        pushRunRecordStack('root.finishedWork = finishedWork');
        try {
...
function requestCurrentTime() {
    pushRunRecordStack('requestCurrentTime');
    try {
...
function requestWork(root, expirationTime) {
    pushRunRecordStack('requestWork');
    try {
...
function addRootToSchedule(root, expirationTime) {
    pushRunRecordStack(`addRootToSchedule`);
    try {
...
function performSyncWork() {
    pushRunRecordStack('performSyncWork');
    try {
...
function performWork(minExpirationTime, isYieldy) {
    pushRunRecordStack('performWork');
    try {
        ...
        if (isYieldy) {
            ...
            while (
              ...
              pushRunRecordStack('...currentRendererTime > nextFlushedExpirationTime');
              try {
                performWorkOnRoot(
                  nextFlushedRoot,
            ...
            while (
              ...
              pushRunRecordStack('nextFlushedRoot !== null && nextFlushedExpirationTime !== NoWork && minExpirationTime <= nextFlushedExpirationTime');
              try {
            ...
            if (_finishedWork !== null) {
                ...
                if (_finishedWork !== null) {
                    ...
                    if (!shouldYield$$1()) {
                        ...
                        pushRunRecordStack('root.finishedWork =  _finishedWork');
                        try {
...
function completeRoot(root, finishedWork, expirationTime) {
    pushRunRecordStack('completeRoot');
    try {
        // dumpFiberTree('completeRoot');
...
function onUncaughtError(error) {
    pushRunRecordStack(`onUncaughtError`);
    try {
        invariant(
            nextFlushedRoot !== null,
        ...
        pushPopRunRecordStack(`expirationTime=(${nextFlushedRoot.id}, ${nextFlushedRoot.expirationTime})`);
        if (!hasUnhandledError) {
            hasUnhandledError = true;
            unhandledError = error;
        }
...
function scheduleRootUpdate(current$$1, element, expirationTime, callback) {
    pushRunRecordStack('scheduleRootUpdate');
    try {
        {
...
function updateContainerAtExpirationTime(
  ...
  callback
    ...
    pushRunRecordStack('updateContainerAtExpirationTime');
    try {
...
function createContainer(containerInfo, isConcurrent, hydrate) {
    pushRunRecordStack('createContainer');
    try {
...
function updateContainer(element, container, parentComponent, callback) {
    pushRunRecordStack('updateContainer');
    try {
        var current$$1 = container.current;
        var currentTime = requestCurrentTime();
    ...
    if (typeof (fiber.type) === 'string') {
      ...
      return fiber.type;
        ...
        if (fiber.type.displayName) {
            // function、ReactElement, 如AppContainer()、App(props)、(props, context, updater)、TouchableText()、{$$typeof: Symbol(react.forward_ref), render: ƒ, displayName: "View"}、
...
function getNativeShortTag(stateNode) {
    if (stateNode) {
        if (stateNode instanceof ReactNativeFiberHostComponent) {
...
function getReactNativeFiberHostComponentTag(nativeFiberHostComponent: ReactNativeFiberHostComponent) {
    if (nativeFiberHostComponent) {
        const {_nativeTag,_children} = nativeFiberHostComponent;
        if (_children) {
            let childrenTag = _children.map((child, index) => (child instanceof ReactNativeFiberHostComponent ? `${getReactNativeFiberHostComponentTag(child)}` : child)).join();
            if (childrenTag) {
...
function dumpFiberTree(desc, currentFiberID = -1) {
    const fibers = [];
    ...
    const doms = instanceCache ? Object.keys(instanceCache).map((key)=> (instanceCache[key])).map((fiber: FiberNode) => {
        ...
        const children = stateNode instanceof ReactNativeFiberHostComponent ? stateNode._children.map((child) => (getNativeShortTag(child))).filter((child) => (child)) : null;
    ...
    const fiberTreeInfo = {
        ...
        desc: `工作${workIndex}-内循环${performWorkIndex}-${desc}`,
        currentFiberID,
        fibers,
        doms,
