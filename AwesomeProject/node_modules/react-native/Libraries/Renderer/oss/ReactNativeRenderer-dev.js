...
var createFiber = function(tag, pendingProps, key, mode) {
  // $FlowFixMe: the shapes are exact here but Flow doesn't like constructors
    pushRunRecordStack('createFiber');
    try {
...
function createFiberFromElement(element, mode, expirationTime) {
    pushRunRecordStack('createFiberFromElement');
    try {
        var owner = null;
        {
...
function beginWork(current$$1, workInProgress, renderExpirationTime) {
    ...
    try {
        ...
        // debugger;
        pushRunRecordStack(tag2String(workInProgress.tag));
        try {
            switch (workInProgress.tag) {
...
function completeWork(current, workInProgress, renderExpirationTime) {
    pushRunRecordStack('completeWork');
    try {
        var newProps = workInProgress.pendingProps;
...
function commitAllHostEffects() {
    ...
    try {
        ...
        while (nextEffect !== null) {
            const desc = 'commitAllHostEffects';
            const currentFiberID = nextEffect.id;
            {
    ...
    try {
        ...
        if (!isYieldy) {
            ...
            while (nextUnitOfWork !== null) {
                ...
                // debugger;
                const desc = `workLoop`;
                const currentFiberID = nextUnitOfWork.id;
...
function travelFiber(fiber: FiberNode, fibers: Array, fiberIDsSet: Set, desc: string = null) {
    if (fiber && !fiberIDsSet.has(fiber.id)) {
        fiberIDsSet.add(fiber.id);
        fibers.push({
            id: fiber.id,
            tag: desc ? `${tag2String(fiber.tag)}(${desc})` : `${tag2String(fiber.tag)}`,
            effectTag: effectTag2String(fiber.effectTag),
            type: getFiberType(fiber),
            nativeTag: getNativeTag(fiber),
            return: fiber.return ? fiber.return.id : -1,
            child: fiber.child ? fiber.child.id : -1,
            sibling: fiber.sibling ? fiber.sibling.id : -1,
            alternate: fiber.alternate ? fiber.alternate.id : -1,
            firstEffect: fiber.firstEffect ? fiber.firstEffect.id : -1,
            lastEffect: fiber.lastEffect ? fiber.lastEffect.id : -1,
            nextEffect: fiber.nextEffect ? fiber.nextEffect.id : -1
        });
        travelFiber(fiber.child, fibers, fiberIDsSet);
        travelFiber(fiber.sibling, fibers, fiberIDsSet);
        travelFiber(fiber.alternate, fibers, fiberIDsSet);
    }
}
...
function dumpFiberTree(desc, currentFiberID) {
    const fibers = [];
    const fiberIDsSet = new Set();
    for (let value of roots.values()) {
        const {current, finishedWork} = value;
        travelFiber(current, fibers, fiberIDsSet, 'current');
        travelFiber(finishedWork, fibers, fiberIDsSet, 'finishedWork');
    }
    // debugger;
